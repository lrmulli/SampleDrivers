  ...
  for k, d in pairs(config.data.device) do
    ...
    for i,cg in pairs(d.controlGroup) do
      for x, action in pairs(cg["function"]) do
        ...
        local a = json.decode(action.action)
        deviceListString = deviceListString..[[{"deviceId":"]]..d.id..[[","command":"]]..a.command..[[","action":"press"}]]..string.char(10)..string.char(13)
      end
    end
  end
  local thisline = ""
...
function connect_ws_harmony(device)
  local hubId = device:get_field("harmony_hub_id")
  local ipAddress = device:get_field("harmony_hub_ip")
  log.info("[" .. device.id .. "] connecting over websockets ip: "..ipAddress)
  device:emit_event(logger.logger("Bridge Device [" .. device.id .. "] - connecting over websockets ip: "..ipAddress))
